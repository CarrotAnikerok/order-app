...
function App() {
  ...
  const [buttonBlocker, setButtonBlocker] = useState<ButtonBlocker>({
    hasNext: false,
    hasPrevious: false,
  });
  ...
  return (
    <div className="font-mono flex flex-col min-h-screen">
      <main className="grow">
        ...
        <div
          className={`flex justify-center items-start transition-all duration-400 ${order ? "gap-5" : "gap-0"}`}
        >
          <div
            className={`m-3 mx-auto transition-all duration-400 flex flex-col items-center gap-2`}
          >
            <OrderList
              orderList={orders}
              setOrder={setOrder}
          ...
          <div
            className={`transition-all duration-400 overflow-auto ${order ? "w-md ml-auto" : "w-0"}`}
          >
            {order ? (
              <OrderInfo order={order} close={() => setOrder(null)}></OrderInfo>
            ) : null}
          </div>
        </div>
      ...
      <footer className="bg-lime-100">
        <div className="flex">
          <p className="flex-1 p-3 text-sm">
            A little fullstack order app with usage of .NET 10, React and SQLite
          </p>
          <p className="flex-1 text-right p-3">
            © 2026{" "}
            <a
              href="https://github.com/CarrotAnikerok"
              target="_blank"
              rel="noreferrer"
